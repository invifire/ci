...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "custom_sidebar":
			...
			$output.= '<input id="add_sidebar" class="button-primary" value="add sidebar"/>';

			if(!empty($custom_sidebars)){
			
				$output.='<h3 id="sidebar_heading" class="heading">'.count($custom_sidebars).' custom sidebars added</h3>';
				foreach($custom_sidebars_array as $sidebar) $output.= '<div class="sidebar_item" data-name="'.$sidebar.'">'.$sidebar.'<input class="button-primary" value="delete"/></div>';
