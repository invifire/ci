...
function create_meta_options($post, $metabox) {
	...
	switch ( $value['type'] ) :
		...
		case 'multiselect_gallery':
		   ...
		   $output .='<select style="height:120px;" name="'.$value['id'].'[]" id="'.$value['id'].'" class="widefat of-input" multiple="multiple">';
		   foreach ($posts_obj as $of_cat):
			  
				$selected='';
					if($post_meta) 
					foreach ($post_meta as $a => $b):

						if($b==$of_cat->ID) $selected = ' selected="selected"';
				
					endforeach;

				$output .= '<option'. $selected .' class="of-input" value="'.$of_cat->ID.'">'.$of_cat->post_name.'</option>';
		...
		case 'gallery_items':
			...
			$output.='<div class="gallery_items_wrap">';
			...
			$output.='<input id="gallery_items" class="of-input hidden" type="text" value="'.$meta_box_value.'" name="gallery_items">';
		...
		case 'upload':
			...
			$output .= '<input name="'.$value['id'].'" value="'.get_meta_option($value['id']).'" />';
